...
for i_prefix in $(timeout $t_vast vast export bro "&type == \"mrt::bgp4mp::update::announcement\"" | bro-cut prefix | sort -u)
do
  #find all prefixes with more specific announcements
  # i_list=$(timeout $t_vast vast export bro "&type == \"mrt::bgp4mp::update::announcement\" && prefix in $i_prefix" | bro-cut prefix | sort -u)
  # size=$(echo "$i_list" | wc -l)
  more_spec=$(timeout $t_vast vast export bro "&type == \"mrt::bgp4mp::update::announcement\" && prefix in $i_prefix")
  ...
  if [ "$n_orig" -ne "1" ]; then
    ...
    while read -r j_prefix; do
      ...
      origins=$(timeout $t_vast vast export bro \
        "&type == \"mrt::bgp4mp::update::announcement\" && prefix == $j_prefix" | \
